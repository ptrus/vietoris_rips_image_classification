...
\end{homeworkSection}
  ...
  \todo{[P] Do the components of the complex V2 distinguish between the three classes? }


\begin{homeworkSection}{Cup, paper, pen image set}
In this section we present the results of Vietoris-Rips classification algorithm described in section~\ref{prob:model}. \todo{Why is the reference not working?}

\begin{paragraph}{Image classification.}  Initially we tested if the model is able to distinguish the already mentioned images of cup, paper and pen. The images were preprocessed, as explained before, with scaling and a PCA transformation. The model had no problems distinguishing between the three classes of images. Each one of the three connected components obtained, was belonging to one of the three classes of images. This is not surprising since looking already mentioned MDS plots~\ref{fig:mds_preprocessing}, the three different classes of points are clearly separable.
\end{paragraph}

\begin{paragraph}{Principal simplices}

  \todo{[P] Do the prinicipal simplices (that is, the simplices which are not faces of any bigger
simplex) of the complex V2 connect similar images? What does the image corresponding
to the barycenter of such a simplex look like?}
