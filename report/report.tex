...
\todo{Do the components of the complex V2 distinguish between the three classes?}


\todo{Explain we get same results as with S-L clustering, and with that the same problems as S-L clustering.}
\todo{Show bad results on our first dataset.}

\todo{Explain that simplices of larger dimension are just about useless}

\todo{Do the prinicipal simplices (that is, the simplices which are not faces of any bigger
...
\todo{Pick one (or several) edges in V1 which is not in V2 and reconstruct the image corresponding
